...
def load_accounts(server_name):
    ...
    try:
        ...
        if not os.path.exists(filename):
            print(f"⚠️ {filename} not found, trying account_pak.txt")
            filename = "account_pak.txt"
            if not os.path.exists(filename):
...
def handle_requests():
    ...
    if not accounts:
        ...
        print(f"⚠️ Using PAK accounts as fallback for {server_name}")
...
if __name__ == '__main__':
    ...
    print("📁 Account files:")
    print("   - account_pak.txt (PAK server)")
